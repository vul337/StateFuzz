...
func objdumpAndSymbolize(obj, arch string) ([]symbolizer.Frame, error) {
	...
	// cmd := osutil.Command("llvm-objdump", "-d", "--no-show-raw-insn", obj)
	log.Logf(0, "start running llvm-objdump")
	...
	log.Logf(0, "callInsn %s, traceFunc %s", callInsn, traceFunc)
	var pcs []uint64
	log.Logf(0, "start s.Scan")
	for s.Scan() {
		ln := s.Bytes()
		// log.Logf(0, "current ln: %s", ln)
		if pos := bytes.Index(ln, callInsn); pos == -1 {
			// log.Logf(0, "callInsn not found, continue")
			continue
		} else if !bytes.Contains(ln[pos:], traceFunc) {
			// log.Logf(0, "traceFunc not found, continue")
			continue
		}
		for len(ln) != 0 && ln[0] == ' ' {
			ln = ln[1:]
		...
		if colon == -1 {
			// log.Logf(0, "colon not found, continue")
			continue
		}
		// log.Logf(0, "parsing pc from %s", ln[:colon])
	...
	log.Logf(0, "s.Scan done, pcs len: %v", len(pcs))
	if len(pcs) != 0 {
		pcchan <- pcs
	}
	close(pcchan)
