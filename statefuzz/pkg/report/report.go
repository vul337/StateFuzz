...
func sanitizeTitle(title string) string {
	...
	tmpRes := strings.TrimSpace(string(res))
	reg := regexp.MustCompile(`CPU: [0-9]* PID: [0-9]*`)
	finalRes := reg.ReplaceAllString(tmpRes, "CPU: NUM PID: NUM")
	return finalRes
}
...
func matchesAny(line []byte, res []*regexp.Regexp) bool {
	if strings.Contains(string(line), "KASAN") || strings.Contains(string(line), "UBSAN") {
		for _, re := range res {
			// log.Logf(0, "matchesAny: %s, re: %s", line, re.String())
			if re.Match(line) {
				// log.Logf(0, "matchesAny hit: %s, re: %s", line, re.String())
				return true
			}
		}
	} else {
