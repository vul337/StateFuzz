...
func RunTest(req *RunRequest, executor string) {
	...
	for run := 0; run < req.Repeat; run++ {
		...
		for i := range info.Calls {
			...
			info.Calls[i].SvSignal = append([]uint32{}, info.Calls[i].SvSignal...)
			info.Calls[i].PcCover = append([]uint32{}, info.Calls[i].PcCover...)
			info.Calls[i].SvCover = append([]uint64{}, info.Calls[i].SvCover...)
		}
		info.Extra.Signal = append([]uint32{}, info.Extra.Signal...)
		info.Extra.SvSignal = append([]uint32{}, info.Extra.SvSignal...)
		info.Extra.PcCover = append([]uint32{}, info.Extra.PcCover...)
		info.Extra.SvCover = append([]uint64{}, info.Extra.SvCover...)
