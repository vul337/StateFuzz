...
func (pool *Pool) Create(workdir string, index int) (vmimpl.Instance, error) {
	...
	// add watcher into android to monitor exception of syz-fuzzer or adb
	if _, err := inst.Copy("/tmp/syzkaller-watcher.sh"); err != nil {
	}
	if _, err := inst.adb("shell", "chmod +x /data/syzkaller-watcher.sh"); err != nil {
		return nil, err
	}
	if _, err := inst.adb("shell", "cd /data; /data/syzkaller-watcher.sh 2>/dev/null 1>/dev/null &"); err != nil {
		return nil, err
	}
