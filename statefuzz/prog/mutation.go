...
func (p *Prog) Mutate(rs rand.Source, ncalls int, ct *ChoiceTable, corpus map[uint32][]*Prog) {
...
// SvMutate : insert calls at end of svSeed
func (p *Prog) SvMutate(rs rand.Source, ncalls int, ct *ChoiceTable, corpus map[uint32][]*Prog) {
	r := newRand(p.Target, rs)
	if ncalls < len(p.Calls) {
		ncalls = len(p.Calls)
	}
...
type mutator struct {
	...
	corpus map[uint32][]*Prog // The entire corpus, including original program p.
}
...
func (ctx *mutator) splice() bool {
	...
	var key uint32
	var bucket []*Prog
	for k := range ctx.corpus {
		keys = append(keys, k)
	}
	if r.Intn(2)%2 != 0 || (len(ctx.corpus[0xdeadbeef]) == 0 && len(ctx.corpus[0xbeefdead]) == 0) {
		keyIdx := r.Intn(len(keys))
		for {
			key = keys[keyIdx%len(keys)]
			bucket = ctx.corpus[key]
			if len(bucket) != 0 {
	...
	} else if (r.Intn(5)%5 != 0 || len(ctx.corpus[0xbeefdead]) == 0) && len(ctx.corpus[0xdeadbeef]) != 0 {
...
// Inserts a new call at a randomly chosen point (with bias towards the end of
// existing program). Does not insert a call if program already has ncalls.
// much prefer inserting at tail of the program
func (ctx *mutator) svInsertCall() bool {
	p, r := ctx.p, ctx.r
	if len(p.Calls) >= ctx.ncalls {
		return false
	}
	idx := r.biasedRand(len(p.Calls)+1, 999)
	var c *Call
	if idx < len(p.Calls) {
		c = p.Calls[idx]
	}
...
// Mutate an argument opf a random call.
