...
func analyze(ct *ChoiceTable, corpus map[uint32][]*Prog, p *Prog, c *Call) *state {
...
func newState(target *Target, ct *ChoiceTable, corpus map[uint32][]*Prog) *state {
	corpusCopy := make(map[uint32][]*Prog)
	for k, m := range corpus {
		corpusCopy[k] = m
	}
	s := &state{
		target:    target,
