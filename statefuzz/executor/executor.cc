...
static uint32* write_output_triple(uint32 v0, uint32 v1, uint32 v2, uint32 v3);
...
{
	// if (sizeof(cover_data_t) == 8) {
	//     printf("cover_data_t size: 8\n");
	// } else if (sizeof(cover_data_t) == 4) {
	//     printf("cover_data_t size: 4\n");
	// }
	int pc_check_res;
	// bool sv_dedup = 0;
	uint32 rw_type_sv_no, sv_value;
	uint32 sv_checksum;
	...
	for (uint32 i = 0; i < cov->size; i++) {
		cover_data_t* pc_ptr = &(cover_data[i]);
		cover_data_t pc = *pc_ptr;
		pc_check_res = cover_check(pc);
		// printf("current pc: %llx, pc_check_res: %d\n", (uint64)pc, pc_check_res);
		if (pc_check_res == 0) {
			debug("got bad pc: 0x%llx\n", (uint64)pc);
			doexit(0);
		}
		// added by sgh0t
		// pc_check_res:  1-->no sv hit, 2-->sv hit, 3-->svInst
		if (pc_check_res == 1) {
			// a sv instrument point pc will be appended behind svCover
			...
			write_output(sig);
			nsig++;
			// printf("send normal pc signal: %llx\n", (uint64)sig);
		} else if (pc_check_res == 3) {
			if (sizeof(cover_data_t) == 8) {
				write_output(pc);
				// printf("send normal sv inst pc: %llx\n", (uint64)pc);
				nsig++;
			}
			(*pc_ptr) = 0;
		}
		// 2: hit sv, no coverage info in pc
		else if (pc_check_res == 2) {
			if (sizeof(cover_data_t) == 8) {
				// printf("cover_data_t size %lud, pc_check_res %d, pc 0x%llx\n", sizeof(cover_data_t), pc_check_res, (uint64)pc);
				// for 64 bit
				// 0x1111:rw_type:sv_no:sv_value = 16:1:15:32
				sv_value = pc & 0xffffffff;
				// cope with right shift count >= width of type
				pc = pc >> 16;
				pc = pc >> 16;
				rw_type_sv_no = pc & 0xffff;
				// hard to use dedup for 64-bit-len sig
				// declare here a sv_info comes

				// 16bit-length, write the sv_info_part1: {rw_type:sv_no}

				// 32bit-length, write the sv_info_part2: {sv_value}
				sv_checksum = hash(0xffffffff) ^ hash(rw_type_sv_no) ^ hash(sv_value);
				write_output_triple(0xffffffff, sv_checksum, rw_type_sv_no, sv_value);
				// printf("send sv signal: 0xffffffff %llx %llx %llx\n", (uint64)sv_checksum, (uint64)rw_type_sv_no, (uint64)sv_value);
				nsig = nsig + 4;
			}
			(*pc_ptr) = 0;
		}
	...
	*signal_count_pos = nsig;
	...
	uint32 cover_size_rel = 0;
	// cover_data has been sorted and avoided dup
	// a sv instrument point pc will be appended behind svCover
	for (uint32 i = 0; i < cover_size; i++) {
		// printf("current pc: %llx\n", (uint64)cover_data[i]);
		// if (cover_check(cover_data[i]) != 2) {
		write_output(cover_data[i]);
		cover_size_rel++;
		// printf("pc cover: %llx\n", (uint64)cover_data[i]);
		// }
	}
	*cover_count_pos = cover_size_rel;
}
#endif
...
uint32* write_output_triple(uint32 v0, uint32 v1, uint32 v2, uint32 v3)
{
	if (output_pos < output_data || (char*)output_pos >= (char*)output_data + kMaxOutput)
		fail("output overflow: pos=%p region=[%p:%p]",
		     output_pos, output_data, (char*)output_data + kMaxOutput);
	uint32* tmp_mem = (uint32*)malloc(sizeof(uint32) * 4);
	uint32* tmp_mem_start = tmp_mem;
	*tmp_mem++ = v0;
	*tmp_mem++ = v1;
	*tmp_mem++ = v2;
	*tmp_mem++ = v3;
	// pthread_spin_lock(&spinlock);
	memcpy(output_pos, tmp_mem_start, 4 * sizeof(uint32));
	output_pos += 4;
	// pthread_spin_unlock(&spinlock);
	return output_pos;
}
