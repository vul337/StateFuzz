...
func main() {
	...
	corpusTmp, err := db.ReadCorpus(*flagCorpus, target)
	corpus := make(map[uint32][]*prog.Prog)
	for _, p := range corpusTmp {
		if _, ok := corpus[p.PathHash]; ok {
			corpus[p.PathHash] = append(corpus[p.PathHash], p)
		} else {
			corpus[p.PathHash] = []*prog.Prog{p}
		}
	}
	if err != nil {
	...
	ct := target.BuildChoiceTableFuzzer(corpus, calls)
		...
		go func() {
			...
			for i := 0; ; i++ {
				...
				if *flagGenerate && len(corpus) == 0 || i%4 != 0 {
					...
					bucket := corpus[key]
					p = bucket[rnd.Intn(len(bucket))].Clone()
					p.Mutate(rs, prog.RecommendedCalls, ct, corpus)
					execute(pid, env, execOpts, p)
					p.Mutate(rs, prog.RecommendedCalls, ct, corpus)
					execute(pid, env, execOpts, p)
