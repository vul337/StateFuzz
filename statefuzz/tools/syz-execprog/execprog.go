...
func (ctx *Context) run(pid int) {
	...
	if err1 != nil {
		log.Fatalf("failed to ImportSvRanges: %v", err1)
	}
	env.SvPairs, err1 = ImportSvPairs()
	if err1 != nil {
		log.Fatalf("failed to ImportSvPairs: %v", err1)
	}
...
func (ctx *Context) printCallResults(info *ipc.ProgInfo) {
	for i, inf := range info.Calls {
		...
		log.Logf(1, "CALL %v: signal %v, sv signal %v, coverage %v, sv_coverage %v, sv_extremum %v, errno %v%v",
			i, len(inf.Signal), len(inf.SvSignal), len(inf.PcCover), len(inf.SvCover), len(inf.SvExtremum), inf.Errno, flags)
	}
}
...
func (ctx *Context) printHints(p *prog.Prog, info *ipc.ProgInfo) {
	...
	for i := range p.Calls {
		...
		svComps := info.Calls[i].SvComps
		for v, args := range svComps {
			ncomps += len(args)
			if *flagOutput {
				fmt.Printf("sv comp 0x%x:", v)
				for arg := range args {
					fmt.Printf(" 0x%x", arg)
				}
				fmt.Printf("\n")
...
func (ctx *Context) dumpCallCoverage(coverFile string, info *ipc.CallInfo) {
	if len(info.PcCover) == 0 {
		return
	}
	buf := new(bytes.Buffer)
	for _, pc := range info.PcCover {
...
func (ctx *Context) dumpCoverage(coverFile string, info *ipc.ProgInfo) {
	for i, inf := range info.Calls {
		log.Logf(0, "call #%v: signal %v, sv signal %v, pc coverage %v, sv coverage %v", i, len(inf.Signal), len(inf.SvSignal), len(inf.PcCover), len(inf.SvCover))
		ctx.dumpCallCoverage(fmt.Sprintf("%v.%v", coverFile, i), &inf)
	}
	log.Logf(0, "extra: signal %v, sv signal %v, pc coverage %v, sv coverage %v", len(info.Extra.Signal), len(info.Extra.SvSignal), len(info.Extra.PcCover), len(info.Extra.SvCover))
...
func ImportSvRanges() (signal.SvRanges, error) {
	...
	if err := json.Unmarshal(bytes, &svRangesJson); err != nil {
		fmt.Println("sv_range Unmarshal: ", err.Error())
		return nil, err
	}
...
func ImportSvPairs() (map[uint32][]uint32, error) {
	...
	if err := json.Unmarshal(bytes, &svPairJson); err != nil {
		fmt.Println("sv_pairs Unmarshal: ", err.Error())
		return nil, err
	}
