...
u32_t ObjTypeInfo::getObjSize(const Value* val) {
    ...
    if (PointerType *pety = SVFUtil::dyn_cast<PointerType>(ety)) {
        ety = pety->getElementType();
    }
#ifdef DEBUG
    llvm::outs() << "u32_t ObjTypeInfo::getObjSize\n";
    if (val != NULL)
        val->print(llvm::outs());
    llvm::outs() << "\ncast type: \n";
    ety->print(llvm::outs());
    if (SVFUtil::isa<PointerType>(ety)) {
        llvm::outs() << "\nis a PointerType\n";
    }
    if (SVFUtil::isa<StructType>(ety))
        llvm::outs() << "\nis a struct type\n";
#endif
    u32_t numOfFields = 1;
...
#ifdef DEBUG
    llvm::outs() << "init ObjTypeInfo: \n";
    val->print(llvm::outs());
    llvm::outs() << "\nobjSize: " << objSize << "  maxOffsetLimit: " << maxOffsetLimit << "\n";
#endif
}
...
#ifdef DEBUG
    llvm::outs() << "setMaxFieldOffsetLimit for Type: ";
    typeInfo->getType()->print(llvm::outs());
    llvm::outs() << "MaxFieldOffsetLimit: " << StInfo::getMaxFieldLimit() << "\n";
#endif 
...
#ifdef DEBUG
    llvm::outs() << "init setMaxFieldOffsetLimit for Type: ";
    if (type != NULL)
