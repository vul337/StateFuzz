...
#ifdef DEBUG    
    llvm::outs() << "we are here\n";
    llvm::outs() << "getGepObjNode(NodeID id, const LocationSet& ls)\n";
#endif
...
#ifdef DEBUG
    llvm::outs() << "NodeID PAG::getGepObjNode(const MemObj* obj, const LocationSet& ls) {\n";
#endif
...
#ifdef DEBUG
    llvm::outs() << "newLS: " << newLS.getOffset() << "\n";
#endif
...
#ifdef DEBUG
    llvm::outs() << "GepObjNodeMap.found: " << iter->second << "\n";
#endif
		return iter->second;
    }
...
NodeID PAG::addGepObjNode(const MemObj* obj, const LocationSet& ls) {
    ...
    if (objNodeSet.count(gepId) == 0) {
#ifdef DEBUG
        llvm::outs() << "objNodeSet insert : " << gepId << "\n";
#endif
        objNodeSet.insert(gepId);
    }
