...
public:
    ...
    inline std::set<NodeID> getObjNodeSet() {
        return objNodeSet;
    }
...
// #ifdef DEBUG
//         llvm::outs() << "isConstantObj(" << id << ") ?\n";
// #endif
        const MemObj* obj = getObject(id);
		assert(obj && "not an object node?");
// #ifdef DEBUG
//         if (SymbolTableInfo::isConstantObj(id))
//             llvm::outs() << "SymbolTableInfo::isConstantObj(id)\n";
//         if (obj->isConstant())
//             llvm::outs() << "obj->isConstant()\n";
// #endif
        return SymbolTableInfo::isConstantObj(id) || obj->isConstant();
	}
    inline bool isNonPointerObj(NodeID id) const {
// #ifdef DEBUG
//     llvm::outs() << " isNonPointerObj(" << id << ")?\n";
// #endif
