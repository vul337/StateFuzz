...
#ifdef DEBUG
            llvm::outs() << "enableDiff\n";
            PointsTo& tmp = getDiffPTDataTy()->getPts(rep);
            for (PointsTo::iterator piter = tmp.begin(), epiter =
                tmp.end(); piter != epiter; ++piter) {
                NodeID ptd = *piter;
                llvm::outs() << "round 1 getPts(" << rep <<"): " << ptd << "\n";
            }
#endif
...
#ifdef DEBUG
                llvm::outs() << "Sanitize pts for field insensitive objects\n";
#endif
...
#ifdef DEBUG
            llvm::outs() << "addCopyEdge : " << src << " --> " << dst << "\n";
#endif
            // 把第二个参数的pts合并到第一个参数的pts中
