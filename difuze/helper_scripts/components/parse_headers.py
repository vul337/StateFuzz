...
def _run_c2xml(c2xml_bin, makeout_file, dst_hdr_file_list, output_file, dst_work_dir=None):
    ...
    count_curr_hdrline = 0
    for curr_hdr_file in all_hdr_files:
        count_curr_hdrline += 1
        dummy_out_file = "/tmp/difuze/dummy_out_" + str(count_curr_hdrline) + ".xml"
            ...
            for curr_s in root:
                if curr_s.get("type") == "struct" and curr_s.get("file") == curr_hdr_file:
                    ...
                    for child_s in curr_s:
                        ...
                        # if child_s.get("ident") == "open":
                        #     output_fp.write('struct.' + str(curr_s.get("ident")) + ',' + str(child_no) + ',FileOpen\n')
                        # if child_s.get("ident") == "mmap":
                        #     output_fp.write('struct.' + str(curr_s.get("ident")) + ',' + str(child_no) + ',MMAP\n')
                        # if child_s.get("ident") == "release":
                        #     output_fp.write('struct.' + str(curr_s.get("ident")) + ',' + str(child_no) + ',RELEASE\n')
                        child_no += 1
        if dst_work_dir is not None:
            os.chdir(back_wd)
    output_fp.close()
