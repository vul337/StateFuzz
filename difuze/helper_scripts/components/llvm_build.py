...
class LLVMBuild(Component):
    ...
    def __init__(self, value_dict):
        ...
        if 'static_checker_name' in value_dict:
            static_checker_name = value_dict["static_checker_name"]
        ...
        self.static_checker_name = static_checker_name
    ...
    def perform(self):
        return _generate_llvm_bitcode(self.kernel_src_dir, self.llvm_bc_out, self.curr_makeout, self.gcc_name,
                                      self.arch_num, self.clang_bin, command_output_dir=self.separate_out,static_checker_name=self.static_checker_name)
...
# UTILITIES FUNCTION
# These flags should be removed from cmdline
INVALID_FLAGS = ['-mno-thumb-interwork', '-fconserve-stack', '-fno-var-tracking-assignments',
                     ...
                     "-DMODULE", "-fsanitize=shadow-call-stack", "--orc", "--static-call"]
...
def _is_allowed_flag(curr_flag):
    ...
    for curr_in_flag in INVALID_FLAGS:
...
def _get_llvm_build_str(src_root_dir, gcc_build_string, output_folder, target_arch, clang_path, build_output_dir=None,static_checker_name=None):
    """
    ...
    gcc_build_strings_tmp = gcc_build_string.split(";")
    for s in gcc_build_strings_tmp:
        complier_name = s.split()[0].split("/")[-1]
        if "gcc" in complier_name or "clang" in complier_name:
            gcc_build_string = s
            break
    ...
    if static_checker_name is not None:
        modified_build_args.append("--analyze")
        modified_build_args.append("-Xclang")
        modified_build_args.append("-analyzer-checker=" + static_checker_name)
        modified_build_args.append("-Xclang")
        modified_build_args.append("-analyzer-checker=" + "debug.SymStateVariableFieldNameAnalysis")
    ...
    #     modified_build_args.append(x86_64_LLVM_ARCH)
    # handle debug flags
    ...
    if static_checker_name is not None:
        modified_build_args.append(" >> " + curr_output_file + ".sv_range")

    if debug:
        print ' '.join(modified_build_args)
...
def _generate_llvm_bitcode(kernel_src_dir, base_output_folder, makeout_file, gcc_prefix, target_arch, clang_path,
                           command_output_dir=None, static_checker_name=None):
    ...
    for curr_line in makeout_lines:
        ...
        if debug:
            print "[+] curr_line: ",
            print curr_line
        fpath = curr_line.split(" ")[-1]
        if debug:
            print "fpath:", fpath
        if static_checker_name is not None:
            if "/drivers/" not in fpath and "/fs/" not in fpath and "/block/" not in fpath and "/net/" not in fpath and "/sound/" not in fpath and \
                not fpath.startswith("drivers/") and not fpath.startswith("fs/") and not fpath.startswith("block/") and not fpath.startswith("net/") and not fpath.startswith("sound/"):
                continue
        if curr_line.startswith(gcc_prefix):
            llvm_mod_str = _get_llvm_build_str(kernel_src_dir, curr_line,
                                               ...
                                               build_output_dir=command_output_dir, static_checker_name=static_checker_name)
            if llvm_mod_str == "":
                continue
