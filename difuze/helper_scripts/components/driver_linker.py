...
class DriverLinker(Component):
    ...
    def setup(self):
        ...
        if self.chipset_numer is None or self.chipset_numer < 1 or self.chipset_numer > 5:
            return "Invalid chipset number. Valid chipset numbers are: 1(mediatek)|2(qualcomm)|3(huawei)|4(samsung)|5(linux_x86)"
        return None
...
#link all file in dir to a big bitcode file
def _process_dir(curr_folder):
    ...
    if os.path.isdir(to_check_folder):
        ...
        if len(all_link_files) > 0:
            ...
            link_list_file = os.path.join(final_output_dir, "link_list_file")
            for cu_d in all_link_files:
                os.system('cp ' + cu_d + ' ' + final_output_dir)
                # os.system('echo "' + curr_folder + "/" + cu_d + '" >> ' + link_list_file)
