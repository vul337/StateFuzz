...
def _run_ioctl_cmd_parser(combined_arg):
    ...
    temp_bc_file.close()
    # run mem2reg
    # if "binder.llvm.bc" in llvm_bc_file:
    #     return 0, func_name
    ...
    '''ret_val = os.system(opt_bin_path + " -analyze -load " + so_path + ' -ioctl-cmd-parser -toCheckFunction=\"' +
                        str(func_name) + '\" ' + bc_file_name + ' > ' + output_file + ' 2>&1') '''
    print opt_bin_path + " -analyze -load " + so_path + ' -new-ioctl-cmd-parser -ioctlFunction=\"' + \
