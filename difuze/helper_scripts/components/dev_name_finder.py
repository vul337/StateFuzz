...
def _run_dev_name_finder(combined_arg):
    ...
    temp_bc_file.close()
    # run mem2reg optimized stack/heap value to register
    print opt_bin_path + " -mem2reg " + llvm_bc_file + " -o " + bc_file_name
    ...
    if ret_val != 0:
        ...
        os.system("cp " + llvm_bc_file + " " + bc_file_name)

    # Old ioctl cmd parser
    print opt_bin_path + " -analyze -load " + so_path + ' -dev-name-finder -ioctlFunction=\"' + str(func_name) + '\" ' + bc_file_name + ' >> ' + output_file + ' 2>&1'
    ret_val = os.system(opt_bin_path + " -analyze -load " + so_path + ' -dev-name-finder -ioctlFunction=\"' +
...
def _run_dev_name_finder_all(entry_point_out, opt_bin_path, ioctl_so_path, ioctl_finder_out):
    ...
    for curr_ep in all_lines:
        #IOCTL:MTK_M4U_ioctl:/difuze/linux/kernel-3.18/out/../drivers/misc/mediatek/m4u/2.0/m4u.c:MTK_M4U_ioctl.txt:/difuze/linux/linux-3.18/kernel-3.18/lvout/drivers/misc/mediatek/m4u/mt6755/llvm_link_final/final_to_check.bc
        curr_ep = curr_ep.strip()
        all_p = curr_ep.split(':')
        # print all_p
        # parse entry_point_out.txt, send correct arguments to dev_finder
        # IOCTL/V4IOCTL records are different from records about Read/Write/Show/Store
        ...
        elif all_p[0] == 'V4IOCTL' and (all_p[1] + all_p[3]) not in processed_func:
            processed_func.append(all_p[1] + all_p[3])
            to_run_cmds.append((opt_bin_path, ioctl_so_path, all_p[1], all_p[-1],
                                os.path.join(v4l2_ioctl_out, all_p[4])))
        elif (all_p[0] == 'DEVOPEN' or all_p[0] == 'DEVRELEASE') and (all_p[1] + all_p[2]) not in processed_func:
            processed_func.append(all_p[1] + all_p[2])
            for i in xrange(len(all_p)):
                if i < 2:
                    continue
                if all_p[i].endswith(".txt"):
                    to_run_cmds.append((opt_bin_path, ioctl_so_path, all_p[1], all_p[-1],
                                    os.path.join(ioctl_finder_out, all_p[i])))
                    break
        elif (all_p[0] == 'FileWrite' or all_p[0] == 'FileRead') and (all_p[1] + all_p[2]) not in processed_func:
            processed_func.append(all_p[1] + all_p[2])
            for i in xrange(len(all_p)):
                if i < 2:
                    continue
                if all_p[i].endswith(".txt"):
                    to_run_cmds.append((opt_bin_path, ioctl_so_path, all_p[1], all_p[-1],
                                    os.path.join(ioctl_finder_out, all_p[i])))
                    break
        # SHOW/STORE interface may interacts with other SHOW/STORE interfaces
        elif (all_p[0] == 'DEVSHOW' or all_p[0] == 'DEVSTORE') and (all_p[1] + all_p[2]) not in processed_func:
            processed_func.append(all_p[1] + all_p[2])
            for i in xrange(len(all_p)):
                if i < 2:
                    continue
                if all_p[i].endswith(".txt"):
                    ...
                    break
        # poll mmap
        elif (all_p[0] == 'DEVPOLL' or all_p[0] == 'DEVMMAP') and (all_p[1] + all_p[2]) not in processed_func:
            processed_func.append(all_p[1] + all_p[2])
            for i in xrange(len(all_p)):
                if i < 2:
                    continue
    ...
    # print "Dev_name_finder Cmd: ", to_run_cmds
    log_info("Found:", len(to_run_cmds), " ioctl/write/read functions to process.")
