...
class IoctlCmd(object):
    ...
    def get_as_json(self):
        to_ret = "{\"cmd_info\":{\"cmd_val\":" + json.dumps(hex(int(self.cmd_val, 10))) + ", \"possible_types\":["
        add_comma = False
...
def parse_device_info(all_lines, ep_object):
    ...
    # added by sgh0t
    if en_idx >= len(all_lines):
        en_idx = len(all_lines) - 1 
...
def parse_type(curr_idx, all_lines):
    ...
    if all_lines[curr_idx].startswith('STARTTYPE'):
        ...
        while not all_lines[curr_idx].startswith('ENDTYPE'):
            if ("ELEMENTS:" not in all_lines[curr_idx]):
...
def process_output_file((ioctl_finder_out, output_json)):
    ...
    try:
        ...
        if ep_name.split("_")[-1].isdigit():
            ep_name = ep_name[:ep_name.rfind("_")]
