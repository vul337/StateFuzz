...
namespace IOCTL_CHECKER {
    ...
    void printTypeRecursive(Type *targetType, llvm::raw_ostream &to_out,
        ...
        if (targetType->isPointerTy()) {
            ...
            printTypeRecursive(
                targetType->getContainedType(0), to_out,
                child_space);
        }
        else if (isa<ArrayType>(targetType)) {
            string child_space = prefix_space;
            child_space = child_space + "  ";
            ArrayType *AT = dyn_cast<ArrayType>(targetType);
            printTypeRecursive(AT->getElementType(), to_out,
                child_space);
        }
            ...
            if(targetType->isStructTy()) {
                ...
                if(src_st_name.find(".anon") != string::npos) {
                    ...
                    to_out << prefix_space << src_st_name << ":ANON STARTELEMENTS:\n";
                    string child_space = prefix_space;
                    child_space = child_space + "  ";
                    for(unsigned int curr_no=0; curr_no<targetType->getStructNumElements(); curr_no++) {
                        // print by adding space
#ifdef DIFUZE_DEBUG
                        to_out << "getStructNumElements: \n";
                        to_out << *(targetType->getStructElementType(curr_no)) << "\n";
#endif
                        printTypeRecursive(targetType->getStructElementType(curr_no), to_out, child_space);
                    }
                    ...
                    if (hasStructElement)
                    {
                        ...
                        for (unsigned int curr_no = 0;
                            ...
                            if (targetType->getStructElementType(curr_no)
                                  ->isStructTy())
                                printTypeRecursive(
                                    targetType->getStructElementType(curr_no), to_out,
                                    child_space);
...
#ifdef DIFUZE_DEBUG
        to_out << "typeOutputHandler(" << *targetVal << ")\n";
#endif
