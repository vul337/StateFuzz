    ...
    public:
        ...
        bool runOnModule(Module &m) override {
            ...
            for(auto a:allStTypes) {
                if(a->hasName() && 
                    (a->getName().str()=="struct.v4l2_ioctl_ops" || a->getName().str().find("struct.v4l2_ioctl_ops.")!=string::npos)
                ) {
                    targetStType = a;
            ...
            for (Module::global_iterator gstart = currGlobalList.begin(), gend = currGlobalList.end();
                ...
                std::string curr_global_name = globalVariable->getName().str();
                if(curr_global_name=="v4l2_ioctls" || curr_global_name.find("v4l2_ioctls.")!=string::npos) {
                    // OK, we got our guy
                        ...
                        if(actualStType != nullptr) {
                            for (unsigned int i = 0; i < actualStType->getNumOperands(); i++) {
                                ...
                                if(valStType->hasName()) {
                                    ...
                                    assert(valStType->getName().str()=="struct.v4l2_ioctl_info" || valStType->getName().str().find("struct.v4l2_ioctl_info.")!=string::npos);
                                }
                                ...
                                if(structVal != nullptr) {
                                    ...
                                    if(targetFunc != nullptr) {
                                        ...
                                        string targetFuncName = targetFunc->getName().str();
