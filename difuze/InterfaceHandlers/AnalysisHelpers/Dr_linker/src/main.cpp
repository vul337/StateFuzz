...
public:
    ...
    bool isNeeded(BitCodeFunctions *dstBCFile) {
        for(std::string &currDecFunction:declaredFunctions) {
            ...
            if (
                currDecFunction.compare("__register_rpmsg_driver") == 0
            )
                continue;
            if(dstBCFile->has_function(currDecFunction)) {
                std::cout << "depending function: " << currDecFunction << "\n";
                return true;
            }
        }
        return false;
...
static const char *huawei_driver_folders[] = {"drivers/connectivity/hisi",
                                      ...
                                      "drivers/vendor/hisi",
                                      "drivers/video/hisi",
                                      "drivers/video/fbdev/hisi/dss",
                                      "drivers/video/fbdev/core",
                                      "drivers/watchdog",
                                      "sound/soc/hisilicon"};
// qualcomm folders
static const char *qualcomm_driver_folders[] = {
...
// Linux driver/net subsystem
static const char *linux_folders[] = {
                                      "drivers/connectivity/hisi",
                                      "drivers/contexthub",
                                      "drivers/devfreq/hisi",
                                      "drivers/dsm",
                                      "drivers/char",
...
// cause mem2reg error in llvm-8
static const char *dont_link[] = {
    "drivers/net",
    "drivers/block",
};
...
int is_interesting_folder(char *curr_folder, unsigned long arch_no) {
    ...
    array_size = SIZEOF_ARRAY(dont_link);
    // skip folders that endswith dont_link[i]
    for (i = 0; i < array_size; i++) {
        if (strlen(curr_folder) >= strlen(dont_link[i])) {
            int s = strlen(curr_folder) - strlen(dont_link[i]);
            if (strcmp(curr_folder + s, dont_link[i]) == 0) {
                std::cout << "[+] Skip Dont_link Folder:" << curr_folder << ": Matched with:" << dont_link[i] << "\n";
                return 0;
            }
        }
    }
    ...
    switch(arch_no) {
        ...
        case 4:
            ...
            break;
        case 5:
            inter_folders = linux_folders;
            array_size = SIZEOF_ARRAY(linux_folders);
            break;
        default:
            std::cerr << "Invalid arch number, Valid arch numbers are: 1(mediatek)|2(qualcomm)|3(huawei)|4(samsung)|5(linux_x86)\n";
            exit(-2);
    }
...
int main(int argc, char *argv[]) {
    ...
    for(i=0; i<all_interesting_files.size(); i++) {
        ...
        std::vector<string> definedFunctions;
        std::vector<string> declaredFunctions;
        std::vector<string> definedSymbols;
        definedFunctions.clear();
        declaredFunctions.clear();
