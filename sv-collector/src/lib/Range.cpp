...
bool RangePass::unionRange(std::string sID, const CRange &R,
						   Value *V = NULL)
{
	if (R.isEmptySet())
		return false;
	...
	if (ConstantStruct *CS = dyn_cast<ConstantStruct>(I)) {
		...
		for (unsigned i = 0; i != ST->getNumElements(); ++i) {
			...
			if (Ty->isStructTy()) {
				...
				std::string sID = getStructId(ST, GV->getParent(), i);
				if (!sID.empty() && CI)
...
CRange RangePass::visitBinaryOp(BinaryOperator *BO)
{
	...
	R.match(L);
	switch (BO->getOpcode()) {
		default: llvm_unreachable("Unknown binary operator!");//BO->dump(); 
